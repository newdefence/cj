<template>
<div class="Home">
    <AForm layout="inline" class="gym-form-inline" @submit.stop>
        <AFormItem>
    ...
    <ATable :loading="j1.loading" :dataSource="j1.data" :rowKey="(row) => row._id.$oid" :columns="[
        ...
        { title: '创建时间', dataIndex: 'createTime', scopedSlots: { customRender: 'datetime' } },
            ...
            :pagination="{ current: j1.current, pageSize: j1.size, total: j1.total, hideOnSinglePage: true }"
            @change="(pagination, filters, sorter)=> { loadJ1(pagination.current); }">
        <template slot="_id" slot-scope="_id">
            <a target='_blank' :href="`/admin/qrcode/${_id.$oid}`">查看二维码</a>
        </template>
        <template slot="startEndTime" slot-scope="_, row">{{ row.startTime | date }} ~ {{ row.endTime | date }}</template>
        <template slot="datetime" slot-scope="ts">{{ ts | datetime }}</template>
    </ATable>
    <Insert v-if="m1" :modal="m1" @success="loadJ1(j1.current)"/>
</div>
</template>
...
export default {
    ...
    data() {
        return {
            ACTIVITY_TYPES,
            j1: { current: 1, size: 20, total: 0, loading: false, data: [] },
            m1: null,
        };
    },
    watch: {
    ...
    methods: {
        loadJ1(current) {
            const vm = this;
            const { j1 } = vm;
            const params = { size: j1.size, current };
            if (j1.current !== current) {
                j1.current = current;
            }
            j1.loading = true;
            vm.$http.get('/admin/activity', { params }).then(({ body: data }) => {
                if (data.success) {
    ...
    components: {
        Insert: antd.Form.create()({
                    ...
                    if (!modal.loading) {
                        form.validateFields((errors, values) => {
                            if (!errors) {
                                ...
                                axios.post('/cms/home/refreshInfo.json', { pid: data.pid, pic: values.pic, seq: values.seq }).then(({ data: body }) => {
                                    if (body.success) {
                                        // data.pic = values.pic;
                                        modal.visible = false;
                                        this.$emit('success');
                                    } else {
                                        this.$notify.warning('首页专辑配置保存失败', body.msg);
                                    }
                                }, (error) => {
                                    this.$notify.error('首页专辑配置保存出错', error);
                                }).finally(() => {
                                    modal.loading = false;
                                });
                            }
            ...
            render() {
                ...
                return <a-modal title="新建活动信息" visible={modal.visible} confirmLoading={modal.loading} onOk={() => {
                    vm.save(modal, form);
                }} onCancel={() => {
                    modal.visible = false;
                }} destroyOnClose>
                    <a-form onSubmit={(e) => {
                        ...
                        <a-form-item {...formItemLayout} label='活动名称'>
                            {
                        ...
                        <a-form-item {...formItemLayout} label='红包总成本'>
                            {
                        ...
                        <a-form-item {...formItemLayout} label='活动红包类型'>
                            {
                        ...
                        <a-form-item {...formItemLayout} label='二维码数量'>
                            {
                        ...
                        {
                            getFieldValue('type') === 'random' ? <a-form-item {...formItemLayout} label='红包总数'>
                                {
                        ...
                        {
                            getFieldValue('type') === 'static' ? <a-form-item {...formItemLayout} label='红包金额配置'>
                                {
                                    getFieldDecorator('config', {
                                    ...
                                    })(<a-text-area rows={4} placeholder={
                        ...
                        {
                            getFieldValue('type') === 'goods' ? <a-form-item {...formItemLayout} label='实物配置'>
                                {
                                    getFieldDecorator('config', {
                                        validateTrigger: 'onBlur',
                                        rules: [
                        ...
                        <a-form-item {...formItemLayout} label='活动开始时间'>
                            {
                        ...
                        <a-form-item {...formItemLayout} label='活动结束时间'>
                            {
